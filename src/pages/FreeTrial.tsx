      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-24">
        <div className="max-w-md w-full text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-red-500/20 bg-red-500/10 text-red-400 text-sm mb-5">
            {t('freeTrial.endedBadge')}
          </div>
          <h1 className="text-3xl font-bold text-white mb-3">{t('freeTrial.endedTitle')}</h1>
          <p className="text-gray-400 text-sm leading-relaxed mb-2">{t('freeTrial.endedBody')}</p>
          <p className="text-gray-500 text-sm leading-relaxed mb-8">{t('freeTrial.endedThanks')}</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/services">
              <button className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-semibold hover:opacity-90 transition-opacity flex items-center gap-2 mx-auto">
                {t('freeTrial.viewServicesBtn')} <FiArrowRight />
              </button>
            </Link>
            <Link to="/">
    ...
    <div className="min-h-screen px-4 py-24">
      <div className="max-w-lg mx-auto">
        ...
        {remaining !== null && (
          <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs text-gray-400">{t('freeTrial.spotsUsed')}</span>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { icon: '🚀', label: t('freeTrial.deliveryTime') },
            { icon: '👥', label: t('freeTrial.followers') },
            { icon: '🆓', label: t('freeTrial.free') },
          ].map((s, i) => (
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          {status === 'success' ? (
            <div className="text-center py-6">
              ...
              <h2 className="text-xl font-semibold text-white mb-2">{t('freeTrial.successTitle')}</h2>
              <p className="text-gray-400 text-sm leading-relaxed mb-2">
                {t('freeTrial.successBody')}
              </p>
              <p className="text-purple-300 font-medium text-sm mb-4 break-all">{link}</p>
              <p className="text-gray-500 text-xs mb-6">{t('freeTrial.successNote')}</p>
              {remaining !== null && remaining < 100 && (
                <p className="text-yellow-400 text-xs mb-4">{t('freeTrial.successLowSpots', { remaining })}</p>
              )}
              <Link to="/services">
                <button className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-semibold hover:opacity-90 transition-opacity flex items-center gap-2 mx-auto">
                  {t('freeTrial.viewServices')} <FiArrowRight />
                </button>
              </Link>
            </div>
          ) : (
            <>
              <div className="mb-4">
                <label className="block text-sm text-gray-400 mb-2">
                  ...
                  {t('freeTrial.usernamePlaceholder')}
                </label>
              ...
              <div className="mb-5">
                ...
                <input
                  ...
                  placeholder={t('freeTrial.linkPlaceholder')}
                  ...
                  disabled={status === 'loading'}
                />
                <p className="text-xs text-gray-600 mt-1.5 ml-1">{t('freeTrial.linkExample')}</p>
              </div>

              {status === 'claimed' && <Alert>{t('freeTrial.alreadyClaimed')}</Alert>}
              {status === 'error' && <Alert>{t('freeTrial.error')}</Alert>}
              ...
              {remaining !== null && remaining <= 50 && remaining > 0 && (
                <div className="flex gap-2 items-center p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-sm mt-3">
                  <span>⚠️</span>
                  <span>{t('freeTrial.urgencyWarning', { remaining })}</span>
                </div>
              )}
              ...
              >
                {status === 'loading'
                  ? <><span className="animate-spin inline-block">⏳</span> {t('freeTrial.loading')}</>
                  : <><FiGift /> {t('freeTrial.submitBtn')} <FiArrowRight /></>
                }
              </button>

              <div className="mt-5 space-y-2 border-t border-white/5 pt-4">
                <div className="flex items-center gap-2 text-gray-500 text-xs"><FiShield className="text-green-500 flex-shrink-0" />{t('freeTrial.rulePublic')}</div>
                <div className="flex items-center gap-2 text-gray-500 text-xs"><FiZap className="text-yellow-500 flex-shrink-0" />{t('freeTrial.ruleSpeed')}</div>
                <div className="flex items-center gap-2 text-gray-500 text-xs"><FiUsers className="text-purple-400 flex-shrink-0" />{t('freeTrial.ruleOnce')}</div>
              </div>
            </>
          )}
        </div>
